import { ACCEPT_TYPE, CHAT_STATUS } from '@constants/Constant'
...
function Chat() {
  ...
  const sendMessage = async (image) => {
    ...
    Fire.shared.onSendMessage(threadId, messages, async (value) => {
      const params = { keyChat: userInfo.key_chat, threadID: threadId }
      ...
      // if (saleInfo.sale_name !== 'Hỗ trợ') {
      await axios
        .get('/socketio', {
          params: {
            type: saleInfo.sale_name !== 'Hỗ trợ' ? CHAT_STATUS.CHAT_TO_ADMIN : CHAT_STATUS.CUSTOMER_CHAT,
            content: 'Bạn có tin nhắn mới từ ' + userInfo.full_name,
          },
        })
  ...
  const resetReadCustomer = async (threadId) => {
    ...
    await Fire.shared.resetReadCustomer(params, (value) => console.log(''))
  }
    ...
    <div className="box-chat">
      ...
      <div>
        <label for="action-chat-box" className="label-chat-box">
          {countUnReadMessage(roomList.concat(roomAdmin)) > 0 && (
