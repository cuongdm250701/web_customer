...
function BookingRoomDetail(props) {
  ...
  const createTransaction = async () => {
    ...
    try {
      await orderApi.createTransaction(
        createFormData({
          order_id: orderId,
          use_point: formatPrice(point) * Number(percent) / 100 || 0,
  ...
  const getUserConfig = async () => {
    try {
      ...
      setPercent(res.data[res.data.length - 1].value)
    } catch (err) {
      swal('Lỗi lấy phần trăm đặt cọc', `${err.msg}`, 'error')
    }
  }
  ...
  useEffect(async () => {
    ...
    getUserInfo()
    getUserConfig()
    if (orderId === Cookies.get('orderUpdateMember')) {
      setTimeout(() => {
        memberRef.current.scrollIntoView({ behavior: 'smooth' })
      }, 500)

      setTimeout(() => {
        Cookies.remove('orderUpdateMember')
      }, 3000)
    }
  }, [])
      ...
      if (
        (Number(orderDetail.payment_status) === PAYMENT_STATUS_NEW.SUCCESS ||
          ...
          Number(orderDetail.payment_status) === PAYMENT_STATUS_NEW.DESPOSIT_SUCCESS) &&
            orderDetail.price > 0 && Number(orderDetail.status) !== ORDER_STATUS.REJECTED 
      ) {
        return (
          <>
        ...
        <div className="col-xl-12 col-xs-12">
          ...
          <div>
            <div style={{ color: 'green', display: 'flex', alignItems: 'center' }}>
              {/* <img src={FingerIcon} className="icon_animation" style={{ width: '30px' }} /> */}
            ...
            {orderDetail?.status != -1 &&
              ...
              (Number(orderDetail.payment_status) === PAYMENT_STATUS_NEW.DESPOSIT_SUCCESS ||
                ...
                <div style={{ display: 'flex', alignItems: 'center' }}>
                  {orderId === Cookies.get('orderUpdateMember') && (
                    <img src={FingerIcon} className="icon_animation" style={{ width: '45px', marginRight: '0.5rem' }} />
                  )}
                  <Button
                    variant="success"
                    onClick={() => {
  ...
  return (
    <Container maxWidth="lg">
      ...
      <div className="mt-4">
        ...
        <div ref={memberRef}>{renderMember()}</div>
      </div>
    </Container>
  )
}
